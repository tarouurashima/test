...
export class TombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    this.TOMB = new ERC20(deployments.tomb.address, provider, 'TEST');
    ...
    // Uniswap V2 Pair
    this.TOMBWFTM_LP = new Contract(externalTokens['TEST-FTM-LP'][0], IUniswapV2PairABI, provider);
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('TEST') ? this.TOMB : this.TSHARE;
    const isTomb = name.startsWith('TEST');
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'TEST' ? await this.getTombStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (depositTokenName.startsWith('TEST')) {
    ...
    } else if (depositTokenName.startsWith('TSHARES')) {
      return rewardPerSecond.mul(15000).div(89500);
    } else {
